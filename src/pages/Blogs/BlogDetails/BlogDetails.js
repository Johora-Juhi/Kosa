...
const BlogDetails = () => {
  ...
  const { data: comments = [], refetch } = useQuery({
    queryKey: ["comments"],
    queryFn: async () => {
      const res = await fetch(`https://hair-saloon-server.vercel.app/comments`);
  ...
  const handleCommentPost = (e) => {
    ...
    fetch("https://hair-saloon-server.vercel.app/comments", {
      method: "POST",
      headers: {
    ...
    <div>
      <div className="backGroundBlogDetails">
        <div className="container mx-auto">
          ...
          <div className="backGroundBlogDetailsTitle text-center">
            {blog.blogTitle}
          </div>
          <div className="backGroundBlogDetailsTimeComment text-center">
            {blog.today} / by KOSATeam /{" "}
            <span>
              {" "}
              <CiChat1></CiChat1> {commentCount}
            </span>{" "}
          </div>
        </div>
      </div>
      ...
      <div className="mb-5">
        <div className="container mx-auto px-5 mb-5">
          {commentCount > 0 && <h1>Comments</h1>}
          <hr />
        </div>
        {comments
          .filter((comment) => comment.blogId === blog._id)
          .map((showComment) => (
            <ShowComment
              key={showComment._id}
      ...
      <div className="container mx-auto blogDetailsComment px-5">
        ...
        <form onSubmit={handleCommentPost}>
          ...
          ></textarea>{" "}
          <br />
          <button type="submit" className="blogButton mt-4">
            Post Comment
          </button>
        </form>
      </div>
